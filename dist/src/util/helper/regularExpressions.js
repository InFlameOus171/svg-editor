...
export const matrixRegExp = new RegExp(`matrix\\(${decimalNumber}(${separator}${decimalNumber}){5}\\)`);
// Matches translate string of a transform
export const translateRegExp = new RegExp(`translate\\(${decimalNumber}(${separator}${decimalNumber})?\\)`);
// Matches scale string of a transform
export const scaleRegExp = new RegExp(`scale\\(${decimalNumber}(${separator}${decimalNumber})?\\)`);
// Matches rotate string of a transform
export const rotateRegExp = new RegExp(`rotate\\(${decimalNumber}((${separator}${decimalNumber}){2})?\\)`);
...
export const hexColorCodeRegExp = new RegExp(/^#?([a-f\d]{2})([a-f\d]{2})([a-f\d]{2})$/i);
